...
def sing(sheet, key='C', ticktime=125, instruments = (), filename='./test.wav'):
    ...
    for track in cues:
        ...
        for (duration, note) in track:
            trackwave.extend(Waves.waveGen(note,duration*ticktime,instrument,vol))
            progress+=1
            if(int(30*progress/notecount)>percent):
                percent=int(30*progress/notecount)
                sys.stdout.write("=")
                sys.stdout.flush()
...
def makeSong(instrument, songname=''):
